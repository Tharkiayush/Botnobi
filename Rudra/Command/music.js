...
module.exports = {
  config: {
    ...
    description: "Choose YouTube song by number (working fix)",
  ...
  run: async function ({ api, event, args }) {
    ...
    try {
      ...
      searchResultsCache[event.threadID] = topResults;
  ...
  handleEvent: async function ({ api, event }) {
    ...
    if (!searchResultsCache[threadID]) return;
    ...
    // ⭐ FIXED API — WORKING
    const apiUrl = `https://api.kenliejugarap.com/download/ytdl?url=${encodeURIComponent(videoUrl)}`;

    delete searchResultsCache[threadID];
    ...
    try {
      ...
      if (!downloadResponse.data || !downloadResponse.data.data || !downloadResponse.data.data.audio) {
        throw new Error("Audio URL nahi mila. 😭");
      }

      const downloadUrl = downloadResponse.data.data.audio;
